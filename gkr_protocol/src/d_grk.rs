...
//     inputs: &[F],
// ) -> Result<GKRProof<F>, Box<dyn Error>> {
...
//     let mut w_0 = circuit_evaluations.first().ok_or("Empty circuit evaluations")?.polynomial.to_vec();
//
//     if w_0.len() == 1 {
//         w_0.push(F::from(0)); // Pad for valid multilinear evaluation
//     }
...
//         .iter()
//         .flat_map(|a| poly_b.iter().map(move |b| op.operation(a, b)))
//         .collect();
//
//     MultilinearPoly::new(new_eval)
// }
//
// // Helper functions for FBC polynomial construction
...
//     let mul_eval_product = ProductPoly::new(vec![mul_i_eval, multiplied_w_poly]);
//
...
//     let (add_i, mul_i) = circuit.add_i_and_mul_i_mle(layer_idx);
//
//     let new_add_i = add_i.multi_partial_evaluate(r_b)?.scale(alpha)
//         + add_i.multi_partial_evaluate(r_c)?.scale(beta);
//
//     let new_mul_i = mul_i.multi_partial_evaluate(r_b)?.scale(alpha)
...
// }
//
// // Helper functions for verifier claim construction
...
//     let mut all_random_challenges = vec![init_random_challenge];
//     all_random_challenges.extend_from_slice(sumcheck_random_challenges);
//
//     let (add_i, mul_i) = circuit.add_i_and_mul_i_mle(layer_idx);
//
//     let a_r = add_i.full_evaluation(all_random_challenges.clone())?;
//     let m_r = mul_i.full_evaluation(all_random_challenges)?;
//
//     Ok((a_r * (o_1 + o_2)) + (m_r * (o_1 * o_2)))
// }
//
...
// }
//
// fn evaluate_input_poly<F: PrimeField>(
//     inputs: &[F],
//     sumcheck_random_challenges: &[F],
// ) -> Result<(F, F), Box<dyn Error>> {
